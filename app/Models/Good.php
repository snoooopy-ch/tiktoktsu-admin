...
class Good extends Authenticatable
{
    ...
    public function getRecordInfoByGoodCode($good_code) {
        $records = DB::connection($this->connection)
            ->table($this->table)
            ->where('good_code', $good_code)
            ->select('*')
    ...
    public function insertRecord($good_code, $info) {
        $id = $this->getRecordByGoodcode($good_code);
        if ($id == -1) {
    ...
    public function getAllForDatatable($params) {
        ...
        if (isset($params['columns'][2]['search']['value'])
            ...
            $selector->where('good_code', 'like', '%' . $params['columns'][2]['search']['value'] . '%');
        }
        if (isset($params['columns'][3]['search']['value'])
            && $params['columns'][3]['search']['value'] !== ''
        ) {
            $selector->where('good_name', 'like', '%' . $params['columns'][3]['search']['value'] . '%');
        }
        if (isset($params['columns'][4]['search']['value'])
            && $params['columns'][4]['search']['value'] !== ''
        ) {
            $selector->where('good_seller', 'like', '%' . $params['columns'][4]['search']['value'] . '%');
        }
    ...
    public function getAllGoodCodes() {
        $records = DB::connection($this->connection)
            ->table($this->table)
            ->select('*')
            ->get();
